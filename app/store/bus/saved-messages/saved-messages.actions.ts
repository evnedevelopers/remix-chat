  ...
  saveMessage: createAsyncThunk(
    ...
    async (
      ...
      try {
        ...
        const response: Response = await fetch(`messages/${payload.messageId}/save`, { method: "POST", body });
  ...
  deleteSavedMessage: createAsyncThunk(
    ...
    async (
      ...
      try {
        ...
        const response: Response = await fetch(`messages/${payload.messageId}/unsave`, { method: 'DELETE' });

        if (!response.ok) {
          return rejectWithValue(response);
        }

        dispatch(projectsActions.removeSavedMessage(payload.messageId));

        if (payload.isLastMessage && payload.isSavedPage) {
          dispatch(savedMessagesActions.removeSavedProject(payload.messageId));
        }

        if (!payload.isLastMessage && payload.isSavedPage) {
          dispatch(savedMessagesActions.removeSavedMessages(payload.messageId));
